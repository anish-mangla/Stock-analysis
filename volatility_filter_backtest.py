#!/usr/bin/env python3
"""
volatility_filter_backtest.py

Quest 3 of 5:
Test whether stock-specific volatility context improves the strategy.

This script:
1. Reads outputs/events.csv
2. Learns GOOD/BAD combos from TRAIN data only
3. Labels TEST events using those learned combos
4. Adds stock volatility features and sector-relative features
5. Tests volatility-based filters and combinations
...
Outputs:
- outputs/volatility_filter_split_summary.csv
- outputs/volatility_filter_all_trades.csv
- outputs/volatility_filter_all_equity.csv
"""
...
VOL_FILTER_SPLIT_SUMMARY_CSV = os.path.join(OUTPUT_DIR, "volatility_filter_split_summary.csv")
VOL_FILTER_ALL_TRADES_CSV = os.path.join(OUTPUT_DIR, "volatility_filter_all_trades.csv")
VOL_FILTER_ALL_EQUITY_CSV = os.path.join(OUTPUT_DIR, "volatility_filter_all_equity.csv")
...
# Sector mapping reused from Quest 2
...
FILTERS: Dict[str, Callable[[pd.Series], bool]] = {
    ...
    # Best sector-relative filter from Quest 2
    "stock_underperformed_sector_by_2pct": lambda row: bool(row["stock_minus_sector_return"] <= -0.02),

    # Volatility-normalized shock filters
    "drop_vs_vol_gt_1_5x": lambda row: bool(row["drop_vs_vol20"] >= 1.5),
    "drop_vs_vol_gt_2_0x": lambda row: bool(row["drop_vs_vol20"] >= 2.0),

    # Avoid extremely volatile names
    "vol20_lt_4pct": lambda row: bool(row["stock_vol20"] < 0.04),
    "vol20_lt_5pct": lambda row: bool(row["stock_vol20"] < 0.05),

    # Combined: stock-specific overreaction relative to own vol
    ...
    "underperformed_sector_2pct_and_drop_vs_vol_2_0x": (
        lambda row: bool(row["stock_minus_sector_return"] <= -0.02) and bool(row["drop_vs_vol20"] >= 2.0)
    ),

    # Combined: prefer calmer names
    "underperformed_sector_2pct_and_vol20_lt_5pct": (
        lambda row: bool(row["stock_minus_sector_return"] <= -0.02) and bool(row["stock_vol20"] < 0.05)
    ),
    "underperformed_sector_2pct_and_vol20_lt_4pct": (
        lambda row: bool(row["stock_minus_sector_return"] <= -0.02) and bool(row["stock_vol20"] < 0.04)
    ),

    # Strongest combo candidate
    "underperformed_sector_2pct_and_drop_vs_vol_1_5x_and_vol20_lt_5pct": (
        lambda row: bool(row["stock_minus_sector_return"] <= -0.02)
        and bool(row["drop_vs_vol20"] >= 1.5)
        and bool(row["stock_vol20"] < 0.05)
    ),
}
...
def build_spy_and_sector_features(start_date: pd.Timestamp, end_date: pd.Timestamp):
    spy_close = download_single_symbol_close("SPY", start_date, end_date)
    ...
    return spy_close, sector_close_map


def build_stock_feature_map(events_df: pd.DataFrame, start_date: pd.Timestamp, end_date: pd.Timestamp) -> Dict[str, pd.DataFrame]:
...
def add_all_features(
    ...
    spy_df["spy_close"] = spy_close
    spy_df["spy_daily_return"] = spy_df["spy_close"].pct_change()
    spy_df = spy_df.reset_index().rename(columns={"Date": "event_date"})
    if "event_date" not in spy_df.columns:
        spy_df = spy_df.rename(columns={spy_df.columns[0]: "event_date"})
    out = out.merge(spy_df, on="event_date", how="left")
...
def run_backtest(events_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    spy_close, sector_close_map = build_spy_and_sector_features(start_date, end_date)
    stock_feature_map = build_stock_feature_map(events_df, start_date, end_date)
    ...
    for split in SPLITS:
        ...
        labeled_test_df = add_all_features(labeled_test_df, spy_close, sector_close_map, stock_feature_map)
...
def save_outputs(
    ...
    trades_df.to_csv(VOL_FILTER_ALL_TRADES_CSV, index=False)
    equity_df.to_csv(VOL_FILTER_ALL_EQUITY_CSV, index=False)
    summary_df.to_csv(VOL_FILTER_SPLIT_SUMMARY_CSV, index=False)

    print(f"\nSaved split summary to: {VOL_FILTER_SPLIT_SUMMARY_CSV}")
    print(f"Saved all trades to:    {VOL_FILTER_ALL_TRADES_CSV}")
    print(f"Saved all equity to:    {VOL_FILTER_ALL_EQUITY_CSV}")
