#!/usr/bin/env python3
"""
INTRADAY V05: Find optimal config with open entry + 0.2% slippage (realistic).
Sweep targets, hold periods, and position sizing together.
"""
import sys, os
...
SLIPPAGE = 0.002  # 0.2% realistic for S&P 100
...
def run_bt(target_ret, max_hold, bull_frac, bear_frac, max_pos, label):
    ...
    for today in trading_dates:
        ...
        for entry in ready:
            ticker = entry['ticker']
            if ticker in positions or len(positions) >= max_pos or cash < entry['size']: continue
    ...
    return {'label': label, 'return': round(total_ret, 2), 'trades': n, 'wr': round(wr, 1), 
            'losers': losers, 'avg_hold': round(avg_hold, 1)}

print("="*60)
print("  INTRADAY V05: OPTIMAL CONFIG (0.2% slippage)")
print("="*60)

results = []

# Top configs from V03/V04 with sizing variations
configs = [
    # (target, hold, bull_frac, bear_frac, max_pos, label)
    (0.07, 30, 6, 14, 12, "+7%/30d, 1/6 bull 1/14 bear"),
    (0.07, 45, 6, 14, 12, "+7%/45d, 1/6 bull 1/14 bear"),
    (0.07, 30, 5, 12, 12, "+7%/30d, 1/5 bull 1/12 bear"),
    (0.07, 30, 4, 10, 12, "+7%/30d, 1/4 bull 1/10 bear"),
    (0.05, 45, 6, 14, 12, "+5%/45d, 1/6 bull 1/14 bear"),
    (0.05, 60, 6, 14, 12, "+5%/60d, 1/6 bull 1/14 bear"),
    (0.10, 30, 6, 14, 12, "+10%/30d, 1/6 bull 1/14 bear"),
    (0.10, 45, 6, 14, 12, "+10%/45d, 1/6 bull 1/14 bear"),
]

for target, hold, bf, brf, mp, label in configs:
    r = run_bt(target, hold, bf, brf, mp, label)
    results.append(r)
    print()

# Save
outdir = os.path.dirname(os.path.abspath(__file__))
with open(os.path.join(outdir, 'v05_optimal_results.json'), 'w') as f:
    json.dump(results, f, indent=2)
with open(os.path.join(outdir, 'v05_optimal_results.txt'), 'w') as f:
    f.write("INTRADAY V05: OPTIMAL CONFIG (0.2% slippage)\n\n")
    for r in results:
        f.write(f"{r['label']}: {r['return']:+.1f}% | {r['trades']} trades | WR {r['wr']:.0f}% | hold {r['avg_hold']:.0f}d\n")
print(f"\nSaved to {outdir}/v05_optimal_results.txt")
