#!/usr/bin/env python3
"""
EXIT V09 OPTIMAL COMBO: Test the +5% target (best from V07/V08) with
different hold periods to find the absolute optimal combination. Also
test +5% target with 120d hold to see if even more patience helps.
"""
import sys, os
...
for hold in [60, 75, 90, 120]:
    results = run_backtest(events, close, high, low, entry_filter,
        target_return=0.05, max_hold_days=hold, position_fraction=10, max_positions=10, entry_delay=2)
    print_results(results, f"EXIT V09: +5% target, {hold}d hold")
    save_results(results, os.path.join(os.path.dirname(__file__), f'v09_5pct_{hold}d_results.txt'))
