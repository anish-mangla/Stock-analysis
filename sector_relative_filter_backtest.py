#!/usr/bin/env python3
"""
sector_relative_filter_backtest.py

Quest 2 of 5:
Test whether sector-relative context improves the strategy.

This script:
1. Reads outputs/events.csv
2. Learns GOOD/BAD combos from TRAIN data only
3. Labels TEST events using those learned combos
4. Adds SPY-based market features and sector ETF features
5. Tests sector-relative and combined market+sector filters
6. Runs portfolio simulations for each split + filter

Inputs:
- outputs/events.csv

Outputs:
- outputs/sector_relative_split_summary.csv
- outputs/sector_relative_all_trades.csv
- outputs/sector_relative_all_equity.csv
"""
...
SECTOR_RELATIVE_SPLIT_SUMMARY_CSV = os.path.join(OUTPUT_DIR, "sector_relative_split_summary.csv")
SECTOR_RELATIVE_ALL_TRADES_CSV = os.path.join(OUTPUT_DIR, "sector_relative_all_trades.csv")
SECTOR_RELATIVE_ALL_EQUITY_CSV = os.path.join(OUTPUT_DIR, "sector_relative_all_equity.csv")
...
# Broad sector ETF mapping
TICKER_TO_SECTOR_ETF: Dict[str, str] = {
    # Tech / software / semis / internet
    ...
    # Utilities / REITs / materials / misc
...
FILTERS: Dict[str, Callable[[pd.Series], bool]] = {
    "good_only_no_context": lambda row: True,

    # Best broad market filter from Quest 1
    "market_spy_not_red_big_today": lambda row: bool(row["spy_daily_return"] > -0.01),

    # Sector-only filters
    "sector_not_red_big_today": lambda row: bool(row["sector_daily_return"] > -0.01),
    "sector_above_50dma": lambda row: bool(row["sector_close_above_50dma"]),
    "sector_above_200dma": lambda row: bool(row["sector_close_above_200dma"]),
    "sector_50dma_above_200dma": lambda row: bool(row["sector_50dma_above_200dma"]),
    "stock_underperformed_sector_by_2pct": lambda row: bool(row["stock_minus_sector_return"] <= -0.02),
    "stock_underperformed_sector_by_3pct": lambda row: bool(row["stock_minus_sector_return"] <= -0.03),

    # Combined sector logic
    "sector_above_50dma_and_stock_underperformed_2pct": (
        lambda row: bool(row["sector_close_above_50dma"]) and bool(row["stock_minus_sector_return"] <= -0.02)
    ),
    "sector_above_200dma_and_stock_underperformed_2pct": (
        lambda row: bool(row["sector_close_above_200dma"]) and bool(row["stock_minus_sector_return"] <= -0.02)
    ),

    # Combined market + sector logic
    "market_ok_and_stock_underperformed_sector_2pct": (
        lambda row: bool(row["spy_daily_return"] > -0.01) and bool(row["stock_minus_sector_return"] <= -0.02)
    ),
    "market_ok_and_sector_above_50dma": (
        lambda row: bool(row["spy_daily_return"] > -0.01) and bool(row["sector_close_above_50dma"])
    ),
    "market_ok_and_sector_above_50dma_and_stock_underperformed_2pct": (
        lambda row: bool(row["spy_daily_return"] > -0.01)
        and bool(row["sector_close_above_50dma"])
        and bool(row["stock_minus_sector_return"] <= -0.02)
    ),
}
...
# ============================================================
# 5) MARKET + SECTOR FEATURES
# ============================================================

def download_single_symbol_features(symbol: str, start_date: pd.Timestamp, end_date: pd.Timestamp, prefix: str) -> pd.DataFrame:
    ...
    if "Close" not in df.columns:
        raise RuntimeError(f"{symbol} download is missing Close column. Columns seen: {list(df.columns)}")
    ...
    out[f"{prefix}_daily_return"] = out[f"{prefix}_close"].pct_change()
    out[f"{prefix}_20d_return"] = out[f"{prefix}_close"].pct_change(20)
    out[f"{prefix}_50dma"] = out[f"{prefix}_close"].rolling(50).mean()
    out[f"{prefix}_200dma"] = out[f"{prefix}_close"].rolling(200).mean()

    out[f"{prefix}_close_above_50dma"] = out[f"{prefix}_close"] > out[f"{prefix}_50dma"]
    out[f"{prefix}_close_above_200dma"] = out[f"{prefix}_close"] > out[f"{prefix}_200dma"]
    out[f"{prefix}_50dma_above_200dma"] = out[f"{prefix}_50dma"] > out[f"{prefix}_200dma"]

    return out


def download_spy_features(start_date: pd.Timestamp, end_date: pd.Timestamp) -> pd.DataFrame:
    return download_single_symbol_features("SPY", start_date, end_date, "spy")


def download_sector_feature_map(start_date: pd.Timestamp, end_date: pd.Timestamp) -> Dict[str, pd.DataFrame]:
    sector_etfs = sorted(set(TICKER_TO_SECTOR_ETF.values()) | {DEFAULT_SECTOR_ETF})
    out: Dict[str, pd.DataFrame] = {}

    for etf in sector_etfs:
        out[etf] = download_single_symbol_features(etf, start_date, end_date, "sector")

    return out


def add_market_and_sector_features(
    events_df: pd.DataFrame,
    spy_df: pd.DataFrame,
    sector_feature_map: Dict[str, pd.DataFrame],
) -> pd.DataFrame:
    out = events_df.copy()
    ...
    # Merge sector ETF features one ETF at a time
    all_parts = []
    for etf, group in out.groupby("sector_etf", sort=False):
        sector_df = sector_feature_map[etf].reset_index().rename(columns={"Date": "event_date"})
    ...
    # Relative performance on event day
...
def filter_events(events_df: pd.DataFrame, filter_name: str) -> pd.DataFrame:
...
def run_backtest(events_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    spy_df = download_spy_features(start_date, end_date)
    sector_feature_map = download_sector_feature_map(start_date, end_date)
    ...
    for split in SPLITS:
        ...
        labeled_test_df = add_market_and_sector_features(labeled_test_df, spy_df, sector_feature_map)
        ...
        print(f"Tradable good-combo test events before extra filter: {len(base_good_test_df)}")

        for filter_name in FILTERS.keys():
            filtered_test_df = filter_events(base_good_test_df, filter_name)
...
def save_outputs(
    ...
    trades_df.to_csv(SECTOR_RELATIVE_ALL_TRADES_CSV, index=False)
    equity_df.to_csv(SECTOR_RELATIVE_ALL_EQUITY_CSV, index=False)
    summary_df.to_csv(SECTOR_RELATIVE_SPLIT_SUMMARY_CSV, index=False)

    print(f"\nSaved split summary to: {SECTOR_RELATIVE_SPLIT_SUMMARY_CSV}")
    print(f"Saved all trades to:    {SECTOR_RELATIVE_ALL_TRADES_CSV}")
    print(f"Saved all equity to:    {SECTOR_RELATIVE_ALL_EQUITY_CSV}")
...
def print_overall_summary(summary_df: pd.DataFrame) -> None:
    ...
    print("\n=== SECTOR RELATIVE FILTER SUMMARY ===")
