#!/usr/bin/env python3
"""
predrop_trend_filter_backtest.py

Quest 4 of 5:
Test whether pre-drop trend / stretch context improves the strategy.
...
4. Adds:
   ...
   - volatility-normalized shock features
   - pre-drop trend / stretch features
5. Tests pre-drop context filters and combinations
...
Outputs:
- outputs/predrop_trend_split_summary.csv
- outputs/predrop_trend_all_trades.csv
- outputs/predrop_trend_all_equity.csv
"""
...
PREDROP_SPLIT_SUMMARY_CSV = os.path.join(OUTPUT_DIR, "predrop_trend_split_summary.csv")
PREDROP_ALL_TRADES_CSV = os.path.join(OUTPUT_DIR, "predrop_trend_all_trades.csv")
PREDROP_ALL_EQUITY_CSV = os.path.join(OUTPUT_DIR, "predrop_trend_all_equity.csv")
...
FILTERS: Dict[str, Callable[[pd.Series], bool]] = {
    "good_only_no_extra_filter": lambda row: True,

    # Best carry-forward filters from prior quests
    "stock_underperformed_sector_by_2pct": lambda row: bool(row["stock_minus_sector_return"] <= -0.02),
    "drop_vs_vol_gt_1_5x": lambda row: bool(row["drop_vs_vol20"] >= 1.5),
    "underperformed_sector_2pct_and_drop_vs_vol_1_5x": (
        lambda row: bool(row["stock_minus_sector_return"] <= -0.02) and bool(row["drop_vs_vol20"] >= 1.5)
    ),

    # Pre-drop trend filters
    "predrop_5d_not_too_weak": lambda row: bool(row["stock_return_5d"] > -0.08),
    "predrop_20d_not_too_weak": lambda row: bool(row["stock_return_20d"] > -0.15),
    "above_20dma": lambda row: bool(row["stock_close_above_20dma"]),
    "above_50dma": lambda row: bool(row["stock_close_above_50dma"]),
    "not_too_far_below_20dma": lambda row: bool(row["pct_from_20dma"] > -0.08),
    "not_too_far_below_50dma": lambda row: bool(row["pct_from_50dma"] > -0.12),

    # Combined pre-drop context
    "predrop_20d_not_too_weak_and_not_too_far_below_20dma": (
        lambda row: bool(row["stock_return_20d"] > -0.15) and bool(row["pct_from_20dma"] > -0.08)
    ),

    # Combine with best prior filter
    "underperformed_sector_2pct_and_predrop_20d_not_too_weak": (
        lambda row: bool(row["stock_minus_sector_return"] <= -0.02) and bool(row["stock_return_20d"] > -0.15)
    ),
    "underperformed_sector_2pct_and_not_too_far_below_20dma": (
        lambda row: bool(row["stock_minus_sector_return"] <= -0.02) and bool(row["pct_from_20dma"] > -0.08)
    ),
    "underperformed_sector_2pct_and_predrop_20d_not_too_weak_and_not_too_far_below_20dma": (
        lambda row: bool(row["stock_minus_sector_return"] <= -0.02)
        and bool(row["stock_return_20d"] > -0.15)
        and bool(row["pct_from_20dma"] > -0.08)
    ),

    # Combine with Quest 3 winner
    "underperformed_sector_2pct_and_drop_vs_vol_1_5x_and_predrop_20d_not_too_weak": (
        lambda row: bool(row["stock_minus_sector_return"] <= -0.02)
        and bool(row["drop_vs_vol20"] >= 1.5)
        and bool(row["stock_return_20d"] > -0.15)
    ),
}
...
def build_base_feature_maps(events_df: pd.DataFrame, start_date: pd.Timestamp, end_date: pd.Timestamp):
    ...
    for ticker in tickers:
        ...
        feat["stock_return_5d"] = feat["stock_close"].pct_change(5)
        feat["stock_return_20d"] = feat["stock_close"].pct_change(20)

        feat["stock_20dma"] = feat["stock_close"].rolling(20).mean()
        feat["stock_50dma"] = feat["stock_close"].rolling(50).mean()

        feat["stock_close_above_20dma"] = feat["stock_close"] > feat["stock_20dma"]
        feat["stock_close_above_50dma"] = feat["stock_close"] > feat["stock_50dma"]

        feat["pct_from_20dma"] = (feat["stock_close"] / feat["stock_20dma"]) - 1.0
        feat["pct_from_50dma"] = (feat["stock_close"] / feat["stock_50dma"]) - 1.0
...
def run_backtest(events_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    sector_close_map, stock_feature_map = build_base_feature_maps(events_df, start_date, end_date)
    ...
    for split in SPLITS:
        ...
        labeled_test_df = add_all_features(labeled_test_df, sector_close_map, stock_feature_map)
...
def save_outputs(
    ...
    trades_df.to_csv(PREDROP_ALL_TRADES_CSV, index=False)
    equity_df.to_csv(PREDROP_ALL_EQUITY_CSV, index=False)
    summary_df.to_csv(PREDROP_SPLIT_SUMMARY_CSV, index=False)

    print(f"\nSaved split summary to: {PREDROP_SPLIT_SUMMARY_CSV}")
    print(f"Saved all trades to:    {PREDROP_ALL_TRADES_CSV}")
    print(f"Saved all equity to:    {PREDROP_ALL_EQUITY_CSV}")
