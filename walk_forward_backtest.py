...
Purpose:
Run honest walk-forward validation for the bounce strategy.

For each train/test split:
- learn GOOD/BAD ticker+bucket combos using TRAIN data only
- apply those learned combo labels to TEST events only
- trade only GOOD combos in the TEST window
- simulate portfolio capital with fixed position sizing

Inputs:
- outputs/events.csv

Outputs:
- outputs/walk_forward_split_summary.csv
- outputs/walk_forward_all_test_trades.csv
- outputs/walk_forward_all_test_equity.csv
"""
...
WALK_FORWARD_SPLIT_SUMMARY_CSV = os.path.join(OUTPUT_DIR, "walk_forward_split_summary.csv")
WALK_FORWARD_ALL_TRADES_CSV = os.path.join(OUTPUT_DIR, "walk_forward_all_test_trades.csv")
WALK_FORWARD_ALL_EQUITY_CSV = os.path.join(OUTPUT_DIR, "walk_forward_all_test_equity.csv")
...
# Useful splits for your current 2021-2024 dataset
SPLITS = [
    {
        "split_name": "train_2021_test_2022",
    ...
    {
        "split_name": "train_2021_2022_test_2023",
        "train_start": "2021-01-01",
    ...
    {
        "split_name": "train_2021_2022_2023_test_2024",
        "train_start": "2021-01-01",
    ...
    {
        "split_name": "train_2022_2023_test_2024",
    ...
    {
        "split_name": "train_2021_2023_test_2024",
        "train_start": "2021-01-01",
        "train_end": "2023-12-31",
        "test_start": "2024-01-01",
        "test_end": "2024-12-31",
...
def simulate_portfolio_for_test_split(
    ...
    summary_df = build_split_summary(split_name, trade_log_df, daily_equity_df)
...
def run_walk_forward(events_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    all_trade_logs = []
    all_equity_curves = []
    all_split_summaries = []
    ...
    for split in SPLITS:
        ...
        tradable_test_df = labeled_test_df[labeled_test_df["combo_label"] == "good"].copy()
        ...
        print(f"Tradable test events (good only): {len(tradable_test_df)}")

        if tradable_test_df.empty:
            print("No tradable good-combo events in test set. Skipping portfolio simulation.")
            continue

        tickers = sorted(tradable_test_df["ticker"].unique().tolist())
        close_matrix = download_close_matrix(
            tickers=tickers,
            start_date=tradable_test_df["event_date"].min(),
            end_date=tradable_test_df["exit_date"].max(),
        )

        trade_log_df, daily_equity_df, split_summary_df = simulate_portfolio_for_test_split(
            split_name=split_name,
            tradable_test_df=tradable_test_df,
    ...
    trade_logs_df = pd.concat(all_trade_logs, ignore_index=True) if all_trade_logs else pd.DataFrame()
    ...
    return trade_logs_df, equity_df, summary_df
...
def save_outputs(
    ...
    trades_df.to_csv(WALK_FORWARD_ALL_TRADES_CSV, index=False)
    equity_df.to_csv(WALK_FORWARD_ALL_EQUITY_CSV, index=False)
    summary_df.to_csv(WALK_FORWARD_SPLIT_SUMMARY_CSV, index=False)

    print(f"\nSaved split summary to: {WALK_FORWARD_SPLIT_SUMMARY_CSV}")
    print(f"Saved all test trades to: {WALK_FORWARD_ALL_TRADES_CSV}")
    print(f"Saved all test equity curves to: {WALK_FORWARD_ALL_EQUITY_CSV}")
...
def main() -> None:
    ...
    trades_df, equity_df, summary_df = run_walk_forward(events_df)
